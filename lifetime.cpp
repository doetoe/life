// g++ --std=c++14 -I. -o lifetime -O3 lifetime.cpp # or clang++
#include <life.h>
#include <random>
#include <iostream>
#include <chrono>
...
int main_timing(int generations, int rows, int cols)
{
    ...
    {
        update_time.push_back(timed_update(&m));
    }

    cout << "World update times (in us): " << endl;
    for (auto t: update_time)
        cout << t << " ";
    cout << endl;

    cout << "min: " << *min_element(update_time.begin(), update_time.end())
         << "us, max: " << *max_element(update_time.begin(), update_time.end())
         << "us, avg: "
         << accumulate(update_time.begin(), update_time.end(), 0) / max(1, int(update_time.size()))
         << "us" << endl;
    return 0;
}
...
int main(int argc, char* argv[])
{
    ...
    else if (argc > 1 and argv[1][0] == 't')
    {
        int generations = (argc > 2) ? atoi(argv[2]) : 10;
        int rows = (argc > 3) ? atoi(argv[3]) : 1000;
        int cols = (argc > 4) ? atoi(argv[4]) : rows;
        return main_timing(generations, rows, cols);
    }
